...
'/*/WP_I18N_DB_CONN_ERROR*/, $dbhost));
	...
	function set_prefix($prefix) {
		...
		if ( preg_match('|[^a-z0-9_]|i', $prefix) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Prefisso database non valido'/*/WP_I18N_DB_BAD_PREFIX*/);
...
<h1>Impossibile selesionare il database</h1>
...
<li>Su alcuni sistemi il nome del database va fatto precedere dal nome utente, così da avere un formato simile a username_wordpress. Potrebbe essere questo il problema?</li>
</ul>
<p>Se non si sa come impostare un database sarebbe opportuno <strong>contattare il proprio host</strong>. Se tutto questo fallisce è possibile trovare aiuto sul <a href=\'http://www.wordpress-it.it/forumt/\'>Forum di supporto italiano su WordPress</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, DB_USER));
			return;
		}
	}
	...
	function print_error($str = '') {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf(/*WP_I18N_DB_QUERY_ERROR_FULL*/'WordPress errore sul database %1$s per la query %2$s fatta da %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller);
		else
			$error_str = sprintf(/*WP_I18N_DB_QUERY_ERROR*/'WordPress errore database %1$s per la query %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query);
	...
	function get_row($query = null, $output = OBJECT, $y = 0) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, output type, int offset) -- Il risultato deve essere uno fra questi: OBJECT, ARRAY_A, ARRAY_N'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
